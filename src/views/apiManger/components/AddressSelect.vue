...
const getProvinceList = () => {
  failedOrder_api.A_apiDistrictList({ platform: props.platform, parentDistrictCode: 0, type: 1 }).then((res: any) => {
    data.provinceList = res as any
  })
}

//市
const getCityList = () => {
  const { provinceId } = props as any
  provinceId &&
    failedOrder_api.A_apiDistrictList({ platform: props.platform, parentDistrictCode: provinceId, type: 2 }).then((res: any) => {
      data.cityList = res
    })
}
...
//区
const getAreaList = () => {
  const { cityId } = props as any
  cityId &&
    failedOrder_api.A_apiDistrictList({ platform: props.platform, parentDistrictCode: cityId, type: 3 }).then((res: any) => {
      data.areaList = res as any
    })
}

const getStreetList = () => {
  const { countyId } = props as any
  countyId &&
    failedOrder_api.A_apiDistrictList({ platform: props.platform, parentDistrictCode: countyId, type: 4 }).then((res: any) => {
      data.streeList = res as any
    })
}
...
<template>
  <el-select v-model="provinceValue" placeholder="省" class="w-150"
    @change="($event: any) => getLabelName($event, 'provinceList', 'provinceName')" filterable>
    <el-option v-for="item in data.provinceList" :key="item.districtId" :label="item.name"
      :value="item.districtId"></el-option>
  </el-select>

  <el-select v-if="provinceValue" v-model="cityValue" class="w-150" placeholder="市" filterable
  ...
  <el-select v-if="cityValue" v-model="areaValue" class="w-150" placeholder="区" filterable
  ...
  <el-select v-if="areaValue && openStreet" v-model="streetValue" placeholder="街道" class="w-150" filterable
